...
create or replace function updateIssue(issue_id int, assignee_name text, new_updatedTime_ timestamp, closedTime_ timestamp,  summary_ text,
description_ text, type_ text, priority_ text, status_  text) returns timestamp
    LANGUAGE plpgsql
as $$
    Declare assignee_id int;
...
Update issues set (authorId, updatedTime, closedTime, summary, description, type, priority, status) =
                      (assignee_id, new_updatedTime_, closedTime_, summary_, description_, type_, priority_, status_)
where  id = issue_id;
end if;
commit;
return old_date;
