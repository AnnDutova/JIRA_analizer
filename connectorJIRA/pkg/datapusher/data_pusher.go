...
func (c *PSQLConnector) PushFirstData(issues []datatransformer.Issue, statuses []datatransformer.IssueStatusChanges) error {
...
func (c *PSQLConnector) UpdateIssue(issue datatransformer.Issue, statuses datatransformer.IssueStatusChanges) error {
	command := fmt.Sprintf("select updateIssue('%d','%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')",
		...
		issue.Summary, issue.Description, issue.Type, issue.Priority, issue.Status)
...
func (c *PSQLConnector) pushIssues(issues []datatransformer.Issue) error {
	for _, val := range issues {
		command := fmt.Sprintf("call insertIssue('%d','%s','%d','%s','%s','%s','%s','%s','%s', '%s', '%s', '%s', '%s', '%s', '%d')",
			...
			val.Summary, strings.ReplaceAll(val.Description, "'", ""), val.Type, val.Priority, val.Status,
			val.TimeSpent)
		_, err := c.db.Exec(command)
		if err != nil {
			return err
